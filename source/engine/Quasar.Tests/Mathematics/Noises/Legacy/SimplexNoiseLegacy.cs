//-----------------------------------------------------------------------
// <copyright file="SimplexNoiseOld.cs" company="Space Development">
//      Copyright (c) Space Development. All rights reserved.
// </copyright>
// <summary>
//     This file is subject to the terms and conditions defined in
//     file 'LICENSE.txt', which is part of this source code package.
// </summary>
// <author>Balazs Meszaros</author>
//-----------------------------------------------------------------------

using System.Runtime.CompilerServices;

using Space.Core.Mathematics;

namespace Quasar.Tests.Mathematics.Noises.Legacy
{
    /// <summary>
    /// Legacy simplex noise implementation.
    /// </summary>
    public static class SimplexNoiseLegacy
    {
        private const double Simplex1DRescale = 0.3961965135;
        private const double Simplex2DRescale = 45.23065;
        private const double F2 = 0.366025403;  // F2 = (sqrt(3) - 1) / 2
        private const double G2 = 0.211324865;  // G2 = (3 - sqrt(3)) / 6

        private static readonly Vector2D[] gradients2D =
        [
            new Vector2D(-1.0, -1.0),
                    new Vector2D(1.0, 0.0),
                    new Vector2D(-1.0, 0.0),
                    new Vector2D(1.0, 1.0),
                    new Vector2D(-1.0, 1.0),
                    new Vector2D(0.0, -1.0),
                    new Vector2D(0.0, 1.0),
                    new Vector2D(1.0, -1.0)
        ];

        private static readonly int[] permutations =
        [
            151, 160, 137, 91, 90, 15, 131, 13,
            201, 95, 96, 53, 194, 233, 7, 225,
            140, 36, 103, 30, 69, 142, 8, 99,
            37, 240, 21, 10, 23, 190, 6, 148,
            247, 120, 234, 75, 0, 26, 197, 62,
            94, 252, 219, 203, 117, 35, 11, 32,
            57, 177, 33, 88, 237, 149, 56, 87,
            174, 20, 125, 136, 171, 168, 68, 175,
            74, 165, 71, 134, 139, 48, 27, 166,
            77, 146, 158, 231, 83, 111, 229, 122,
            60, 211, 133, 230, 220, 105, 92, 41,
            55, 46, 245, 40, 244, 102, 143, 54,
            65, 25, 63, 161, 1, 216, 80, 73,
            209, 76, 132, 187, 208, 89, 18, 169,
            200, 196, 135, 130, 116, 188, 159, 86,
            164, 100, 109, 198, 173, 186, 3, 64,
            52, 217, 226, 250, 124, 123, 5, 202,
            38, 147, 118, 126, 255, 82, 85, 212,
            207, 206, 59, 227, 47, 16, 58, 17,
            182, 189, 28, 42, 223, 183, 170, 213,
            119, 248, 152, 2, 44, 154, 163, 70,
            221, 153, 101, 155, 167, 43, 172, 9,
            129, 22, 39, 253, 19, 98, 108, 110,
            79, 113, 224, 232, 178, 185, 112, 104,
            218, 246, 97, 228, 251, 34, 242, 193,
            238, 210, 144, 12, 191, 179, 162, 241,
            81, 51, 145, 235, 249, 14, 239, 107,
            49, 192, 214, 31, 181, 199, 106, 157,
            184, 84, 204, 176, 115, 121, 50, 45,
            127, 4, 150, 254, 138, 236, 205, 93,
            222, 114, 67, 29, 24, 72, 243, 141,
            128, 195, 78, 66, 215, 61, 156, 180
        ];


        /// <summary>
        /// Samples 1D noise value.
        /// </summary>
        /// <param name="x">The sampling x value.</param>
        /// <returns>The sampled noise.</returns>
        public static double Sample(double x)
        {
            var i0 = MathematicsHelper.FastFloor(x);
            var i1 = i0 + 1;
            var x0 = x - i0;
            var x1 = x0 - 1.0;

            var t0 = 1.0 - x0 * x0;
            var t20 = t0 * t0;
            var t40 = t20 * t20;

            var t1 = 1.0 - x1 * x1;
            var t21 = t1 * t1;
            var t41 = t21 * t21;

            var n0 = t40 * Gradient(Hash(i0)) * x0;
            var n1 = t41 * Gradient(Hash(i1)) * x1;

            // The maximum value of this noise is 8*(3/4)^4 = 2.53125
            // A factor of 0.3961965135 scales to fit exactly within [-1,1]
            return Simplex1DRescale * (n0 + n1);
        }

        /// <summary>
        /// Samples the 2D simplex noise at the specified x, y position.
        /// </summary>
        /// <param name="x">The x coordinate.</param>
        /// <param name="y">The y coordinate.</param>
        /// <returns>The sampled simplex noise value.</returns>
        public static double Sample(in double x, in double y)
        {
            double n0, n1, n2;   // Noise contributions from the three corners

            // Skewing/Unskewing factors for 2D

            // Skew the input space to determine which simplex cell we're in
            var s = (x + y) * F2;  // Hairy factor for 2D
            var xs = x + s;
            var ys = y + s;
            var i = MathematicsHelper.FastFloor(xs);
            var j = MathematicsHelper.FastFloor(ys);

            // Unskew the cell origin back to (x,y) space
            var t = (i + j) * G2;
            var x0 = x - (i - t);  // The x,y distances from the cell origin
            var y0 = y - (j - t);

            // For the 2D case, the simplex shape is an equilateral triangle.
            // Determine which simplex we are in.
            int i1, j1;  // Offsets for second (middle) corner of simplex in (i,j) coords
            if (x0 > y0)
            {
                // lower triangle, XY order: (0,0)->(1,0)->(1,1)
                i1 = 1;
                j1 = 0;
            }
            else
            {
                // upper triangle, YX order: (0,0)->(0,1)->(1,1)
                i1 = 0;
                j1 = 1;
            }

            // A step of (1,0) in (i,j) means a step of (1-c,-c) in (x,y), and
            // a step of (0,1) in (i,j) means a step of (-c,1-c) in (x,y), where
            // c = (3-sqrt(3))/6
            var x1 = x0 - i1 + G2;              // Offsets for middle corner in (x,y) unskewed coords
            var y1 = y0 - j1 + G2;
            var x2 = x0 - 1.0 + 2.0 * G2;       // Offsets for last corner in (x,y) unskewed coords
            var y2 = y0 - 1.0 + 2.0 * G2;

            // Work out the hashed gradient indices of the three simplex corners
            var gi0 = Hash(i + Hash(j));
            var gi1 = Hash(i + i1 + Hash(j + j1));
            var gi2 = Hash(i + 1 + Hash(j + 1));

            // Calculate the contribution from the first corner
            var t0 = 0.5 - x0 * x0 - y0 * y0;
            if (t0 < 0.0)
            {
                n0 = 0.0;
            }
            else
            {
                t0 *= t0;
                n0 = t0 * t0 * Gradient(gi0, x0, y0);
            }

            // Calculate the contribution from the second corner
            var t1 = 0.5 - x1 * x1 - y1 * y1;
            if (t1 < 0.0)
            {
                n1 = 0.0;
            }
            else
            {
                t1 *= t1;
                n1 = t1 * t1 * Gradient(gi1, x1, y1);
            }

            // Calculate the contribution from the third corner
            var t2 = 0.5 - x2 * x2 - y2 * y2;
            if (t2 < 0.0)
            {
                n2 = 0.0;
            }
            else
            {
                t2 *= t2;
                n2 = t2 * t2 * Gradient(gi2, x2, y2);
            }

            // Add contributions from each corner to get the final noise value.
            // The result is scaled to return values in the interval [-1,1].
            return Simplex2DRescale * (n0 + n1 + n2);
        }


        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static double Gradient(int hash)
        {
            var h = hash & 0x0F;
            var grad = 1.0 + (h & 7);
            if ((h & 8) != 0)
            {
                grad = -grad;
            }

            return grad;
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static double Gradient(int hash, double x, double y)
        {
            var h = hash & 7;
            return gradients2D[h].X * x + gradients2D[h].Y * y;
            ////var h = hash & 0x3F;
            ////var u = h < 4 ? x : y;
            ////var v = h < 4 ? y : x;
            ////return ((h & 1) > 0 ? -u : u) + ((h & 2) > 0 ? -2.0f * v : 2.0f * v);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static int Hash(int index)
        {
            return permutations[(byte)index];
        }
    }
}
