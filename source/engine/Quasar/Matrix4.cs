//-----------------------------------------------------------------------
// <copyright file="Matrix4.cs" company="Space Development">
...
namespace Quasar
{
    ...
    /// </summary>
    /// <seealso cref="IEquatable{Matrix4x4}" />
    ...
    public unsafe struct Matrix4 : IEquatable<Matrix4>
    {
        ...
        public Matrix4(in Vector4 row0, in Vector4 row1, in Vector4 row2, in Vector4 row3)
        {
            cells[0] = row0.X;
            cells[1] = row0.Y;
            cells[2] = row0.Z;
        ...
        /// The zero matrix.
        /// </summary>
        public static readonly Matrix4 Zero = default;
        ...
        public void FromQuaternion(in Quaternion quaternion, bool rowMajor)
        {
            if (rowMajor)
            {
        ...
        /// <summary>
        /// Initializes the matrix from the scale vector.
        /// </summary>
        ...
        {
            float a = cells[0], b = cells[4], c = cells[8], d = cells[12];
            float e = cells[1], f = cells[5], g = cells[9], h = cells[13];
            float i = cells[2], j = cells[6], k = cells[10], l = cells[14];
            float m = cells[3], n = cells[7], o = cells[11], p = cells[15];
            ...
            if (MathF.Abs(determinant) < MathematicsConstants.Epsilon)
            {
        ...
        public static void Multiply(in Matrix4 left, in Matrix4 right, ref Matrix4 result)
        {
        ...
        public void Transpose(ref Matrix4 result)
        {
            var k = 0;
            for (var i = 0; i < N; i++)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private unsafe void CopyCells(in Matrix4 source)
        {
            fixed (float* targetPointer = cells)
            {
                fixed (float* sourcePointer = source.cells)
                {
                    Unsafe.CopyBlock(targetPointer, sourcePointer, N2 * sizeof(float));
                }
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private unsafe void FromQuaternionColumnMajor(in Quaternion quaternion)
        {
            ...
            cells[0] = 1.0f - 2.0f * (yy + zz);
            cells[1] = 2.0f * (xy + wz);
            cells[2] = 2.0f * (xz - wy);
            cells[3] = 0.0f;

            cells[4] = 2.0f * (xy - wz);
            cells[5] = 1.0f - 2.0f * (zz + xx);
            cells[6] = 2.0f * (yz + wx);
            cells[7] = 0.0f;

            cells[8] = 2.0f * (xz + wy);
            cells[9] = 2.0f * (yz - wx);
            cells[10] = 1.0f - 2.0f * (yy + xx);
            cells[11] = 0.0f;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private unsafe void FromQuaternionRowMajor(in Quaternion quaternion)
        {
            var xx = quaternion.X * quaternion.X;
            var yy = quaternion.Y * quaternion.Y;
            var zz = quaternion.Z * quaternion.Z;
